...
dopamine_task_dict = {'Task1': 'a Fam Rew',
                      'Task2': 'b No Rew',
...
def runSVM(animalinfo, animalname, classifier_type='Bayes'):
    ...
    current_task_dict = dopamine_task_dict
    data = LoadData(FolderName=animalinfo['foldername'], animalname=animalname,
                    Task_NumFrames=animalinfo['task_numframes'],
                    og_task_dict=animalinfo['task_dict'], TaskDict=current_task_dict,
                    framestokeep=animalinfo['task_framestokeep'],
                    v73_flag=animalinfo['v73_flag'])

    bin_behdata = PrepareBehaviorData(BehaviorData=data.good_running_data, TaskDict=current_task_dict,
    ...
    for keys in current_task_dict.keys():
        end_of_lapframes = np.where(data.good_lapframes[keys] == np.max(data.good_lapframes[keys]))[0][-1]
        end_of_lapframes = np.size(data.good_lapframes[keys]) - end_of_lapframes
        ...
        if 'Task2' in keys:
            ...
            lapframes[keys] = data.good_lapframes[keys][data.lickstopframe:]

            Xdata['Task2all'] = data.Fc3data_dict[keys][:, data.good_running_index[keys]].T[
                                :animalinfo['task_framestokeep'][keys], :]
            Ydata['Task2all'] = bin_behdata.position_binary[keys][
                                :animalinfo['task_framestokeep'][keys]]
            lapframes['Task2all'] = data.good_lapframes[keys][:animalinfo['task_framestokeep'][keys]]
        else:
            Xdata[keys] = data.Fc3data_dict[keys][:, data.good_running_index[keys]].T[
                          :animalinfo['task_framestokeep'][keys], :]
            Ydata[keys] = bin_behdata.position_binary[keys][:animalinfo['task_framestokeep'][keys]]
            lapframes[keys] = data.good_lapframes[keys]
    ...
    for t in current_task_dict.keys():
    ...
    plot_some_stuff(taskmodel, current_task_dict, trackbins=animalinfo['trackbins'], tracklength=animalinfo['tracklength'], )

    # Save SVM models from each dataset
    np.save(os.path.join(animalinfo['saveresults'], 'modeloneachtask_lapwise'), taskmodel)
    return taskmodel


def plot_some_stuff(taskmodel, task_dict, trackbins, tracklength):
    fs, ax = plt.subplots(2, len(task_dict), figsize=(10, 6), sharex='all', sharey='all')
    for n, t in enumerate(task_dict.keys()):
...
class LoadData(object):
    def __init__(self, FolderName, animalname, Task_NumFrames, og_task_dict, TaskDict, framestokeep, v73_flag):
        print('Loading Data')
        self.FolderName = FolderName
        self.Task_Numframes = Task_NumFrames
        self.animalname = animalname
        self.TaskDict = TaskDict
        self.OG_TaskDict = og_task_dict
        ...
        self.lickstopframe = np.where(self.good_lapframes['Task2'] == self.lickstoplap['Task2'])[0][0]
    ...
    def load_fluorescentdata(self):
        self.Fcdata_dict = CommonFunctions.create_data_dict(self.OG_TaskDict)
        self.Fc3data_dict = CommonFunctions.create_data_dict(self.OG_TaskDict)
        ...
        count = 0
        for i in self.OG_TaskDict.keys():
...
class PrepareBehaviorData(object):
    ...
    def convert_y_to_index(self, Y, task, trackstart_index=0, figure_flag=1):
        ...
        strange_transition = self.fix_strange_transition(Y_binary)

        if strange_transition.size:
            print('Transitions in..%s: %d' % (task, np.size(strange_transition)))
            Y_binary[strange_transition + 1] = 0

        if figure_flag:
            fs = plt.figure(figsize=(10, 2))
            plt.plot(Y_binary)
            plt.plot(strange_transition, Y_binary[strange_transition + 1], 'r*', markersize=10)
            plt.title(task)
    ...
    def fix_strange_transition(self, Y_data):
        diff = np.diff(Y_data)
        point_of_trans = np.where(diff > 30)[0]
        return point_of_trans
