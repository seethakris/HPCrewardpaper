...
def runmodel(animalinfo, classifier_type='Bayes'):
    ...
    savedmodel = np.load(os.path.join(animalinfo['saveresults'], 'modeloneachtask_lapwise_unfiltered.npy'), allow_pickle=True).item()
...
def save_data_fit_params(SaveFolder, animalname, decoderfit, data, classifier_type='Bayes'):
    np.savez(os.path.join(SaveFolder, f'%s_decoderfit_with_cntrl_unfiltered_%s.npz' % (animalname, classifier_type)),
